     ...
     (let [arg (cond
       ...
       (try
         ...
         (catch #?(:clj Exception :cljs js/Error) e
           (log/errorf "Error during transaction %s" #?(:clj (.getMessage e) :cljs (str e)))
...
;; deprecation begin
   ...
   (extend-protocol IConfiguration
     ...
     (-create-database [config #_& #_deprecated-config]
        (println "Point: 1 before load-config")
       (let [{:keys [keep-history? initial-tx] :as config} (dc/load-config config nil #_deprecated-config)
             store-config (:store config)
             _  (println "Point: 1" store-config)
             atom-store (atom (cache/lru-cache-factory {} :threshold 1000))
             _  (println "Point: 1.1" @atom-store)
             empty-store (ds/empty-store store-config)
             _  (println "Point: 1.2" empty-store)
             store (kc/ensure-cache
                    empty-store
                    atom-store
                    #_(atom (cache/lru-cache-factory {} :threshold 1000)))
             _  (println "Point: 2")
             stored-db (<?? S (k/get-in store [:db]))
             _ (when stored-db
