         ...
         {'hitchhiker.tree.bootstrap.konserve.KonserveAddr
          (fn [{:keys [last-key konserve-key]}]
            (konserve-addr store
                           last-key
                           konserve-key))
          'hitchhiker.tree-cljs.DataNode               ;;TODO: this is temporary fix for changes made in hh-tree for cljs
          (fn [{:keys [children cfg] :as d}]
            (tree/data-node (into (sorted-map-by c/-compare)
                                  children)
                            cfg))
          'hitchhiker.tree-cljs.IndexNode
